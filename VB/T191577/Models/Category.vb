...
'INSTANT VB NOTE: The variable category was renamed since it may cause conflicts with calls to static members of the user-defined type with this name:
				Dim category_Renamed As New Category() With {.CategoryID = DirectCast(row("CategoryID"), Integer), .CategoryName = DirectCast(row("CategoryName"), String), .Description = DirectCast(row("Description"), String), .Picture = DirectCast(row("Picture"), Byte()), .Icon_17 = DirectCast(row("Icon_17"), Byte()), .Icon_25 = DirectCast(row("Icon_25"), Byte())}
